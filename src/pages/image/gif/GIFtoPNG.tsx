    ...
    decodedFrames.forEach((frame, index) => {
      ...
      canvas.toBlob(blob => {
        if (blob) {
          ...
          newFrames.push({
            blobUrl,
            name: `${gifFile.name.replace(/\.gif$/i, "")}_frame_${index + 1}.png`,
          });

          if (newFrames.length === decodedFrames.length) {
            setFrames(newFrames);
            toast({
              title: `${newFrames.length} frame(s) extracted`,
              description: "Each frame is now available as a PNG",
            });
            setIsLoading(false);
          }
        }
    ...
    <>
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>GIF to PNG Converter</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="max-w-4xl mx-auto">
        <Card className="shadow-md">
          <CardHeader className="bg-primary/5">
            <div className="flex items-center">
              ...
              <CardTitle>GIF to PNG Converter</CardTitle>
            </div>
            <CardDescription>Extract frames from GIF and convert to PNG</CardDescription>
          </CardHeader>
          ...
          <CardContent className="pt-6 pb-4 space-y-4">
            ...
            {frames.length > 0 && (
              <>
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-medium">{frames.length} Frame{frames.length > 1 ? "s" : ""} Extracted</h3>
            ...
            <p className="text-sm text-muted-foreground">
              Tip: This tool extracts individual frames from a GIF and converts each to a PNG image.
            </p>
          </CardContent>
        ...
        <GuidanceSection title="How to Use the GIF to PNG Converter">
          <div className="space-y-4">
            <div>
              <h4 className="font-medium mb-1">Steps</h4>
              <ol className="list-decimal pl-5">
                <li>Select a GIF file to extract its frames.</li>
                <li>Each frame will be converted to a PNG image.</li>
                <li>Preview or download any frame.</li>
                <li>Use "Clear All" to reset the tool.</li>
              </ol>
            </div>
            ...
            <div>
              ...
              <ul className="list-disc pl-5">
                <li>Extract all frames from animated GIFs</li>
                <li>Convert each frame into a high-quality PNG</li>
                <li>No uploads — fully client-side</li>
              </ul>
            </div>
          </div>
        </GuidanceSection>
