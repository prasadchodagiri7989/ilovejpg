...
import {
  ...
  BreadcrumbPage,
  BreadcrumbSeparator,
>>>>>>> 6f207f81d143164e7e16dcaacb9caf0d26f1d28b
...
=======
  const [isLoading, setIsLoading] = useState(false);
 const [images, setImages] = useState<Array<{ file: File; preview: string; dataUrl: string; width: number; height: number }>>([]);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { toast } = useToast();
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          ...
          if (processedCount === validSVGs.length) {
            setImages((prev) => [...prev, ...newImages]);
            toast({
              title: `${newImages.length} SVG image(s) ready`,
        ...
        img.onerror = () => {
          toast({
            title: `Failed to render SVG: ${file.name}`,
            variant: "destructive",
          });
          processedCount++;
          if (processedCount === validSVGs.length) {
  ...
  const downloadPDF = (index: number) => {
    ...
    const pdf = new jsPDF({
      ...
      unit: "px",
      format: [img.width || 612, img.height || 792]
// default to A4 size
    });
    ...
    pdf.addImage(img.dataUrl, "JPEG", 0, 0, pdfWidth, pdfHeight);
    pdf.save(img.file.name.replace(/\.svg$/i, ".pdf"));
  };
      ...
      <div className="max-w-4xl mx-auto">
        <Card className="shadow-md">
          <CardHeader className="bg-primary/5">
            ...
            <CardDescription>Convert SVG images to PDF format</CardDescription>
          </CardHeader>
          ...
          <CardContent className="pt-6 pb-4 space-y-4">
            ...
            {images.length > 0 && (
              <>
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-medium">{images.length} Processed Image{images.length > 1 ? "s" : ""}</h3>
                  <Button variant="outline" size="sm" onClick={clearAll}>
                    <Trash2 size={16} className="mr-1" /> Clear All
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {images.map((img, index) => (
                    <div key={index} className="relative group border rounded-md overflow-hidden">
                      <img src={img.dataUrl} alt={`Preview ${index + 1}`} className="w-full h-32 object-cover" />
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        <Button
                          size="icon"
                          variant="ghost"
                        ...
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={(e) => { e.stopPropagation(); downloadPDF(index); }}
                          className="h-8 w-8 text-white hover:bg-white/20"
                        >
                          <Download size={16} />
                        </Button>
            ...
            <p className="text-sm text-muted-foreground">
              Tip: This tool converts your SVG images into high-resolution PDF documents.
            </p>
          </CardContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <ol className="list-decimal pl-5">
                ...
                <li>Each file is rendered into a high-resolution image.</li>
                <li>Then, it's embedded into a PDF using jsPDF.</li>
                <li>Click download icon to save your PDF file.</li>
              </ol>
            </div>
            ...
            <div>
              <h4 className="font-medium mb-1">Features</h4>
              <ul className="list-disc pl-5">
                <li>Client-side SVG to PDF conversion</li>
                <li>Supports multiple files and previews</li>
                <li>No uploads — fully privacy safe</li>
              </ul>
            </div>
          </div>
        </GuidanceSection>
