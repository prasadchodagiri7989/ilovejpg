...
export const PNGtoSVG = () => {
        ...
        img.onload = () => {
          ...
          const svgContent = `
            <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
              <image href="${reader.result}" width="${width}" height="${height}" />
            </svg>
          `.trim();
          ...
          if (processedCount === validImages.length) {
            ...
            toast({
              ...
              description: "Now downloadable as SVG",
            });
            setIsLoading(false);
          }
        };
  ...
  const downloadImage = (index: number) => {
    ...
    a.download = images[index].file.name.replace(/\.png$/i, ".svg");
    a.click();
  };
    ...
    <>
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>PNG to SVG Converter</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="max-w-4xl mx-auto">
        <Card className="shadow-md">
          <CardHeader className="bg-primary/5">
            <div className="flex items-center">
              ...
              <CardTitle>PNG to SVG Converter</CardTitle>
            </div>
            <CardDescription>Convert PNG images to SVG format (Base64 embedded)</CardDescription>
          </CardHeader>
          ...
          <CardContent className="pt-6 pb-4 space-y-4">
                  ...
                  {images.map((img, index) => (
                    <div key={index} className="relative group border rounded-md overflow-hidden">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-xs p-1 truncate">
                        {img.file.name.replace(/\.png$/i, ".svg")}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <p className="text-sm text-muted-foreground">
              Tip: PNGs are wrapped inside SVGs using Base64 encoding. Useful for responsive web design or embedding.
            </p>
          </CardContent>
        ...
        <GuidanceSection title="How to Use the PNG to SVG Converter">
          <div className="space-y-4">
            <div>
              <h4 className="font-medium mb-1">Step-by-step Instructions</h4>
              <ol className="list-decimal pl-5">
                <li>Click "Select PNG Files" or drag-and-drop images.</li>
                <li>Each PNG is wrapped inside an SVG format in the browser.</li>
                <li>Use the preview (eye icon) or download individual SVGs.</li>
                <li>Click "Clear All" to remove images and reset.</li>
              </ol>
            </div>
            <div>
              <h4 className="font-medium mb-1">Features</h4>
              <ul className="list-disc pl-5">
                <li>No upload — fully browser-based and secure.</li>
                <li>Wraps PNG inside an SVG for flexible use in HTML/CSS.</li>
                <li>Fast, batch-conversion supported, same clean UI.</li>
              </ul>
            </div>
          </div>
        </GuidanceSection>
