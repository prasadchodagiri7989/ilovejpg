...
// 👇 Use require and declare module workaround for TypeScript
import heic2any from "heic2any";

export const HEICtoPNG = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsLoading(false);
    toast({ title: `${converted.length} image(s) converted`, description: "Now downloadable as PNG" });
  ...
  const downloadImage = (i: number) => {
    ...
    a.download = images[i].file.name.replace(/\.(heic|heif)$/i, ".png");
    a.click();
  };
    ...
    <>
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbPage>HEIC to PNG Converter</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="max-w-4xl mx-auto">
        <Card className="shadow-md">
          <CardHeader className="bg-primary/5 flex items-center">
            ...
            <CardTitle>HEIC to PNG Converter</CardTitle>
            <CardDescription>Convert HEIC/HEIF images to PNG format</CardDescription>
          </CardHeader>
                  ...
                  {images.map((img, i) => (
                    <div key={i} className="relative group border rounded-md overflow-hidden">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-xs p-1 truncate">
                        {img.file.name.replace(/\.(heic|heif)$/i, ".png")}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <GuidanceSection title="How to Use the HEIC to PNG Converter">
          <ol className="list-decimal pl-5">
            ...
            <li>Preview and download each PNG from the grid.</li>
