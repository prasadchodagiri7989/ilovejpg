...
export const HEICtoJPG = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsLoading(false);
    toast({ title: `${converted.length} image(s) converted`, description: "Now downloadable as JPG" });
  ...
  const downloadImage = (i: number) => {
    ...
    a.download = images[i].file.name.replace(/\.(heic|heif)$/i, ".jpg");
    a.click();
  };
    ...
    <>
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbPage>HEIC to JPG Converter</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="max-w-4xl mx-auto">
        <Card className="shadow-md">
          <CardHeader className="bg-primary/5 flex items-center">
            ...
            <CardTitle>HEIC to JPG Converter</CardTitle>
            <CardDescription>Convert HEIC/HEIF images to JPG format</CardDescription>
          </CardHeader>
                  ...
                  {images.map((img, i) => (
                    <div key={i} className="relative group border rounded-md overflow-hidden">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-xs p-1 truncate">
                        {img.file.name.replace(/\.(heic|heif)$/i, ".jpg")}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <GuidanceSection title="How to Use the HEIC to JPG Converter">
          <ol className="list-decimal pl-5">
            ...
            <li>Preview and download each JPG from the grid.</li>
